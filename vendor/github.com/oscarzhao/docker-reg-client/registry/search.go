...
type SearchResultsBackup struct {
...
func (s *SearchService) Query(query string, page, num int) (*SearchResults, error) {
	...
	if err != nil {
		resultsBackup := &SearchResultsBackup{}
		if _, err = s.client.do(req, resultsBackup); err != nil {
			return nil, err
		} else {
			return &SearchResults{
				NumPages:   resultsBackup.NumPages,
				NumResults: resultsBackup.NumResults,
				Results:    resultsBackup.Results,
				PageSize:   resultsBackup.PageSize,
				Query:      resultsBackup.Query,
				Page:       resultsBackup.Page,
			}, nil
		}
	}
